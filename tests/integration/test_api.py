...
class TestSecretsAPI:
    ...
    def test_create_secret(self, client, auth_headers, db):
        ...
        response = client.post(
            "/api/v1/secrets", json=secret_data, headers=auth_headers
        )
    ...
    def test_get_secret(self, client, auth_headers, test_secret):
        ...
        response = client.get(f"/api/v1/secrets/{test_secret.id}", headers=auth_headers)
    ...
    def test_get_secret_not_found(self, client, auth_headers):
        ...
        response = client.get("/api/v1/secrets/nonexistent_id", headers=auth_headers)

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_get_secret_unauthorized(
        self, client, auth_headers, test_secret, test_user_data, db
    ):
        ...
        response = client.get(
            f"/api/v1/secrets/{test_secret.id}", headers=other_headers
        )
    ...
    def test_update_secret(self, client, auth_headers, test_secret):
        ...
        update_data = {"name": "updated_api_key", "value": "new_secret_value_xyz"}

        response = client.put(
            f"/api/v1/secrets/{test_secret.id}", json=update_data, headers=auth_headers
        )
    ...
    def test_delete_secret(self, client, auth_headers, test_secret):
        ...
        response = client.delete(
            f"/api/v1/secrets/{test_secret.id}", headers=auth_headers
        )
    ...
    def test_pagination(self, client, auth_headers, test_user, db):
        ...
        # Test first page
        response = client.get(
            "/api/v1/secrets?page=1&per_page=10", headers=auth_headers
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        ...
        # Test second page
        response = client.get(
            "/api/v1/secrets?page=2&per_page=10", headers=auth_headers
        )
        data = response.json()
